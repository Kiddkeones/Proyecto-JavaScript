...
for (const key in productsFromLocalStorage) {
  if (productsFromLocalStorage.hasOwnProperty(key)) {
    ...
    compraButton.classList.add("compra-btn");
    compraButton.textContent = "Realizar Compra"
    ...
    productDetails.appendChild(compraButton);
...
function realizarCompra() {
  ...
  localStorage.clear();
  const product = cart.querySelector(".product");
  product.innerHTML = " "
  const totalElement = cart.querySelector(".total");
totalElement.innerHTML = `<p>Total: $0</p>`;
  ...
  Swal.fire({
    ...
    text: "Esperamos que disfrutes de tu producto.",
    icon: "success",
